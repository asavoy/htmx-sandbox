...
async def inline_edit(request: Request):
    return templates.TemplateResponse("inline_edit.html", {"request": request, "users": users})
...
async def update_user(request: Request, user_id: int, name: str = Form(...), email: str = Form(...)):
    user = next((u for u in users if u["id"] == user_id), None)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    if "@" not in email:
        return templates.TemplateResponse(
            "user_edit_form.html", 
            {"request": request, "user": {"id": user_id, "name": name, "email": email}, "error": "Invalid email format"}
        )
    ...
    response_content.headers["HX-Trigger"] = json.dumps({"showMessage": "User updated successfully"})
    
    return response_content

@app.get("/oob-swaps", response_class=HTMLResponse)
async def oob_swaps(request: Request):
    return templates.TemplateResponse("oob_swaps.html", {"request": request, "users": users})
...
async def trigger_event(request: Request):
    ...
    response.headers["HX-Trigger"] = json.dumps({"myCustomEvent": {"message": "Hello from the server!", "timestamp": time.time()}})
    return response
...
async def event_stream():
    async def generate():
        ...
        while True:
            ...
            count += 1
            message_data = json.dumps({'message': f'Notification {count}', 'timestamp': time.time()})
...
async def slow_response(request: Request):
    ...
    return templates.TemplateResponse("slow_response.html", {"request": request, "data": "This data was loaded after 3 seconds"})
...
async def submit_modal(request: Request, name: str = Form(...), email: str = Form(...)):
    ...
    if "@" not in email:
        return templates.TemplateResponse(
            "modal_form.html", 
            {"request": request, "name": name, "email": email, "error": "Invalid email format"}
        )
